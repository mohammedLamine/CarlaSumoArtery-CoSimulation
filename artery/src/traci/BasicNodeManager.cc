...
void BasicNodeManager::processVehicles()
{
    ...
    for (auto& vehicle : m_vehicles) {
        ...
        bool exist_id=false;
        for (const std::string& traciid : m_api->vehicle.getIDList()) {
            if (traciid==id) {
                exist_id =true;
                break;
            }
        }
        // if (!std::find(std::begin(m_api->vehicle.getIDList()), std::end(m_api->vehicle.getIDList()), id) != std::end(m_api->vehicle.getIDList())){
        // m_vehicles.erase(id);
        if (!exist_id) {
            std::cout<< "erasing id "<< id <<"\n";
            unknown_id=id;
            // m_vehicles.erase(id);
        }else{
    ...
    m_vehicles.erase(unknown_id);
    removeNodeModule(unknown_id);
}
